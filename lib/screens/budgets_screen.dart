...
class _BudgetsTab extends StatelessWidget {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(title, style: Theme.of(context).textTheme.bodySmall?.copyWith(color: cs.onSurfaceVariant), textAlign: TextAlign.center, maxLines: 1),
            const SizedBox(height: 4),
            FittedBox(
              fit: BoxFit.scaleDown,
              child: Text(NumberFormat.currency(symbol: '৳').format(amount), style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.bold, color: color)),
            ),
          ],
        ),
      ),
  ...
  Widget _buildBudgetCard(BuildContext context, Budget budget, BudgetProvider budgetProvider) {
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                CircleAvatar(
                  ...
                  child: Icon(_getCategoryIcon(budget.category), color: _getCategoryColor(budget.category)),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('${NumberFormat.currency(symbol: '৳').format(budget.spent)} / ${NumberFormat.currency(symbol: '৳').format(budget.limit)}', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: Theme.of(context).colorScheme.onSurface.withOpacity(0.6))),
                    ],
                  ),
                ),
            ...
            Row(
              children: [
                Expanded(
                  child: LinearProgressIndicator(value: progressValue, backgroundColor: progressColor.withOpacity(0.2), valueColor: AlwaysStoppedAnimation<Color>(progressColor), minHeight: 8),
                ),
                const SizedBox(width: 12),
                Text('${budget.progressPercentage.toStringAsFixed(1)}%', style: TextStyle(fontWeight: FontWeight.bold, color: progressColor)),
              ],
            ),
            if (budget.isOverBudget) ...[
              ...
              Container(
                ...
                child: Text('Over budget by ${NumberFormat.currency(symbol: '৳').format(budget.spent - budget.limit)}', style: const TextStyle(color: Colors.red, fontSize: 12, fontWeight: FontWeight.bold)),
              ),
            ],
          ],
        ),
  ...
  Widget _buildEmptyState(BuildContext context) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(Icons.account_balance_wallet, size: 80, color: Theme.of(context).colorScheme.onSurface.withOpacity(0.3)),
          const SizedBox(height: 16),
          Text('No budgets yet', style: Theme.of(context).textTheme.titleLarge?.copyWith(color: Theme.of(context).colorScheme.onSurface.withOpacity(0.5))),
          const SizedBox(height: 8),
          Text('Create your first budget to start tracking your spending', style: Theme.of(context).textTheme.bodyMedium?.copyWith(color: Theme.of(context).colorScheme.onSurface.withOpacity(0.4)), textAlign: TextAlign.center),
        ],
      ),
    );
  }
        ...
        content: Form(
          ...
          child: TextFormField(
            ...
            validator: (value) {
              ...
              if (double.parse(value) <= 0) return 'Limit must be greater than 0';
              return null;
            },
          ),
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              if (formKey.currentState!.validate()) {
                final budgetProvider = Provider.of<BudgetProvider>(context, listen: false);
                final updatedBudget = budget.copyWith(limit: double.parse(limitController.text));
                budgetProvider.updateBudget(updatedBudget);
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Budget updated successfully')));
              }
            },
